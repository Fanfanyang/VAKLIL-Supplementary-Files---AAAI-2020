...
class ARGS():
    def __init__(self):
        ...
        self.r_margin = 1
        # LS-GAIL
        self.slope = 0.0    #'slope for function c proposed in generalized lsgan, when slope is 0, gls-gan is lsgan, when slope is 1 gls-gan is AL.'
        ...
        self.LSGAN = False
        # save path
        self.description = 'VAKLIL',
        self.save_data_path = proj_direc+"build/data/"+self.env_name+"_VAKLIL.pkl"
args = ARGS()
...
def create_networks():
    ...
    if args.AL:
        ...
        discrim_net = AESNDiscriminator(state_dim + action_dim, hidden_size=(32,), encode_size=64, activation='leakyrelu',slope=0.1,dropout=True,dprob=0.2)
    elif args.VAKLIL:
        noise_dim = 64
        mid_dim = 32
        discrim_net = VAEDiscriminator(state_dim + action_dim, num_outputs=noise_dim, sigmoid_out=False, sn = True, test=False, w_init=False, hidden_size_enc=(), hidden_size_dec=(), encode_size=mid_dim, activation='relu',dropout=False)
    ...
    def update_params(batch, i_iter):
        ...
        for _ in range(args.discriminator_epochs):
            ...
            if args.EBGAN or args.GMMIL or args.VAKLIL:
                g_o_enc,g_mu,g_sigma = discrim_net(dis_input_fake,mean_mode=False)
            ...
            if args.AL:
                ...
                noise_num = 20000
                mmd2_D_net,_,penalty = mix_imp_with_bw_mmd2(e_o_enc,g_o_enc,noise_num, noise_dim, kernel_net, cuda,args.sigma_list)
                ...
                i_c = 0.2
                bottleneck_loss = torch.mean((0.5 * torch.sum((torch.cat((e_mu, g_mu), dim=0) ** 2) + (torch.cat((e_sigma, g_sigma), dim=0) ** 2)
                                      - torch.log((torch.cat((e_sigma, g_sigma), dim=0) ** 2) + 1e-8) - 1, dim=1))) - i_c  
                discrim_loss = -errD + (args.beta * bottleneck_loss) + (args.lambda_h * penalty)
            else:
        ...
        if args.VAKLIL:
            with torch.no_grad():
                ...
                _,K_net,_ = mix_imp_with_bw_mmd2(e_o_enc,g_o_enc,noise_num, noise_dim, kernel_net, cuda,args.sigma_list)
    ...
    for e_iter in range(args.epochs):
        ...
        for i_iter in range(args.max_iter_num):
            ...
            if i_iter % args.log_interval == 0:
                ...
                print('{}\tT_sample {:.2f}\tT_update {:.2f}\tR_tot {:.2f}\tR_avg {:.2f} +- {:.2f}\t R_std {:.2f}\tEpisodes {:.2f}\tSteps {:.2f}\tBeta {:.4f}'.format(
                    i_iter, log['sample_time'], t1 - t0, log['total_reward'], r_mean, r_dif, r_std,\
                    log['num_episodes'], log['num_steps'], args.beta))
            rList.append(r_mean)
